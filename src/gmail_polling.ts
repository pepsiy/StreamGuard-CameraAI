...
export class GmailIdleService {
    ...
    private connect() {
        ...
        this.imap.once('error', (err: Error) => {
            ...
            setTimeout(() => {
                console.log('[Gmail] Reconnecting...');
                this.connect();
            }, 10000);
        });
    ...
    private extractEmailData(parsed: any): EmailData | null {
        ...
        if (attachments && attachments.length > 0) {
            ...
            const imageAtt = attachments.find((att: any) =>
                att.contentType?.startsWith('image/')
            );

            if (imageAtt) {
                console.log(`[Gmail] ✅ Got image from attachment: ${imageAtt.filename || 'unnamed'}`);
                return {
                    from,
                    subject,
                    text,
                    imageBuffer: imageAtt.content,
                    imageFilename: imageAtt.filename || 'snapshot.jpg'
                };
            }
        }
