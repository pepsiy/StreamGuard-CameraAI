...
// import { openRouterService } from './src/ai/openrouter'; // Removed
import { yoloService } from './src/ai/yolo_service';
...
// import { keyManager } from './src/ai/key_manager'; // Removed
...
app.post('/api/settings', (req, res) => {
    const { huggingfaceToken, aiProvider, telegramToken, telegramChatId, telegramAdminId, aiEnabled } = req.body;
    ...
    try {
        ...
        // updates['DAILY_QUOTA'] = dailyQuota; // Removed
...
// API: Key Usage APIs Removed (Legacy)
...
app.post('/api/zones', (req, res) => {
    try {
        ...
        console.log('[Zones] Updated zones configuration');
        res.json({ success: true });
    } catch (error) {
...
function extractCameraNameFromSubject(subject: string): string {
    // if (subject.includes('Camera Motion Test')) return 'Camera 01'; // Removed default
...
if (gmailUser && gmailPassword) {
    if (gmailPassword.includes('your_app_password')) {
        ...
        gmailService.start(async (emailData: EmailData) => {
            ...
            // Save latest snapshot for Dashboard Zone Editor
            if (emailData.imageBuffer) {
                try {
                    const publicDir = path.join(process.cwd(), 'public');
                    if (!fs.existsSync(publicDir)) fs.mkdirSync(publicDir);
                    const savedPath = path.join(publicDir, 'latest.jpg');
                    fs.writeFileSync(savedPath, emailData.imageBuffer);
                    console.log(`[Server] 💾 Saved latest snapshot to ${savedPath}`);
                } catch (e) {
                    console.error('[Server] ❌ Failed to save latest.jpg:', e);
                }
            }
            ...
            try {
                ...
                // AI Analysis (YOLOv8 on Hugging Face Spaces)
                console.log(`[AI] Using provider: YOLOv8 (HF Spaces)`);
                ...
                } catch (e) { console.error('[Zone] Read error', e); }

                const zonePoints = zones[cameraName];
                if (zonePoints) console.log(`[Zone] Applied zone for ${cameraName}: ${zonePoints.length} points`);

                const filterConfig = getFilterConfig();

                const result = await yoloService.analyzeSnapshot(
                    emailData.images || emailData.imageBuffer,
                    cameraName,
                    zonePoints,
                    {
                        personIouThreshold: filterConfig.personIouThreshold,
                        vehicleIouThreshold: filterConfig.vehicleIouThreshold,
                        ignoreMovingPersons: filterConfig.ignoreMovingPersons,
                    }
                );
                ...
                if (result.shouldAlert) {
                    ...
                    // Trust YOLO's decision - no additional filtering needed
                    // YOLO has already applied zone filtering, IOU analysis, and detection thresholds
